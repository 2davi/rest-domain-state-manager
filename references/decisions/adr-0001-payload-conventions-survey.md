...
- **점유율 추정:** ~10-15%. 표준은 명확하지만 백엔드 채택 의외로 낮음. 프레임워크 라이브러리 미흡.
- **대표 사용처:** Microsoft API, 일부 엔터프라이즈 자바, Kubernetes API.
- **본 라이브러리 fit:** ★★★★★ — 현재 default. changeLog가 직접적으로 매핑됨.

### Option 2 — RFC 7396 JSON Merge Patch

> **HTTP PATCH 다른 표준. 변경된 부분만 객체로 보내고, `null`은 삭제 시그널.**
...
- **점유율 추정:** ~30-40%. 6902보다 직관적이라 더 보편적.
- **대표 사용처:** GitHub API, OpenAPI 권장, Django REST Framework 기본 PATCH 동작.
- **본 라이브러리 fit:** ★★★★★ — 6902가 너무 verbose하다고 느끼는 백엔드의 첫 선택지.
...
- **점유율 추정:** ~5-10%. Ember.js 생태계, 일부 Rails 프로젝트.
- **대표 사용처:** Ember Data, jsonapi-rb, Drupal JSON:API module.
- **본 라이브러리 fit:** ★★★ — envelope 패턴의 대표. 명세가 엄격해서 빌트인 가치 있음.
...
- **점유율 추정:** ~5-10%. Spring HATEOAS 채택 영역.
- **대표 사용처:** Spring HATEOAS, Apache Camel, RESTful API 학술 영역.
- **본 라이브러리 fit:** ★★ — 응답 파싱은 의미 있으나 요청 페이로드는 일반 JSON. 차별화 약함.
...
- **점유율 추정:** 한국 엔터프라이즈 ~50%, 글로벌 자바 ~30%.
- **대표 사용처:** Spring Data REST를 채택한 모든 자바 백엔드.
- **본 라이브러리 fit:** ★★★★ — "PATCH 안 쓰고 PUT 전체 교체" 패턴의 대표. fullPut 프리셋의 모태.
...
- **점유율 추정:** Python 백엔드 ~40%.
- **대표 사용처:** Django REST Framework 채택한 모든 Python 백엔드.
- **본 라이브러리 fit:** ★★ — 사실상 RFC 7396 Merge Patch와 동일. 별도 프리셋 불필요.
...
- **본 라이브러리 fit:** ★★★ — envelope 변형. 일반화된 envelope strategy로 커버 가능.
...
- **점유율 추정:** 한국 SI 사업장 ~70-80%.
- **대표 사용처:** 대다수 한국 SI 프로젝트, JSP+Spring 환경.
- **본 라이브러리 fit:** ★★★★★ — 본 라이브러리의 핵심 페르소나(SI 트랙). 빌트인 필수.
...
- **점유율 추정:** ~5%. Microsoft .NET 생태계.
- **대표 사용처:** Microsoft Graph, Dynamics 365, SAP.
- **본 라이브러리 fit:** ★★ — 매우 전용적. metadata 처리·query 문법 등 페이로드 직렬화 외 책임 다수.
...
- **본 라이브러리 fit:** ★ — 도메인 전용. 일반 라이브러리에 빌트인할 가치 없음.
...
- **점유율 추정:** ~3%. Google 영향권 일부.
- **대표 사용처:** Envoy proxy, Google Cloud, 일부 마이크로서비스.
- **본 라이브러리 fit:** ★ — REST가 아닌 RPC. 본 라이브러리의 영역 밖.
...
- **본 라이브러리 fit:** ★ — 전용성 높음. GraphQL 라이브러리(Apollo)의 영역.
...
| # | 프리셋 명 | 모태 컨벤션 | 채택 근거 |
| :---: | --- | --- | --- |
| 1 | `rfc6902()` | Option 1 | 표준 PATCH 페이로드. changeLog가 직접 매핑됨. 본 라이브러리의 기술적 정체성. |
| 2 | `mergePatch()` | Option 2 | 가장 보편적인 PATCH 컨벤션. Django/GitHub 등 광범위 커버. |
| 3 | `fullPut()` | Option 5 | 한국 엔터프라이즈/글로벌 자바의 표준. PATCH 안 쓰는 백엔드 커버. |
| 4 | `postOnly()` | Option 8 | 한국 SI의 핵심 페르소나. 라이브러리 SOM의 결정적 확장 요소. |
| 5 | `jsonApi()` | Option 3 | envelope 패턴의 spec 기반 대표. Ember/Drupal/Rails 일부. |
| 6 | `envelope({ wrap, unwrap })` | Option 7 일반화 | Rails ActiveResource + 사용자 정의 envelope 모두 커버하는 일반화 프리셋. |
...
| Option | 기각 사유 |
| :---: | --- |
| Option 4 (HAL) | 응답 파싱 위주, 요청 페이로드는 일반 JSON. 차별화 약함. 사용자 정의 strategy로 충분. |
| Option 6 (Django REST) | 사실상 Option 2(Merge Patch)와 동일. mergePatch 프리셋으로 커버. |
| Option 9 (OData) | 도메인 전용성 높음. metadata/query 처리는 strategy 외 책임. 사용자 정의 strategy 가이드로 안내. |
| Option 10 (FHIR) | 헬스케어 도메인 전용. 일반 라이브러리에 빌트인할 가치 없음. |
| Option 11 (gRPC-Web) | RPC 패턴, 본 라이브러리의 REST 영역 밖. |
| Option 12 (GraphQL-over-REST) | niche 패턴. GraphQL 라이브러리의 영역. |
...
- ✅ 시장 점유율 합산 80% 이상 커버 (RFC 6902 + Merge Patch + Spring + 한국 SI + JSON:API + envelope).
- ✅ 한국 시장(SI POST-only)과 글로벌 시장(RESTful 표준) 양쪽 커버.
- ✅ 6종으로 번들 크기 부담 낮음. Tree-shaking 시 사용자가 import한 프리셋만 포함.
- ✅ envelope 일반화 프리셋이 Rails ActiveResource + 사용자 정의 envelope을 모두 흡수.
- ❌ JSON:API와 envelope이 일부 기능적으로 겹침. 사용자가 어느 것을 선택해야 할지 혼란.
- ❌ Spring HATEOAS, OData, FHIR 사용자는 자체 strategy 작성 필요 (그러나 이들은 niche).
...
- ✅ 번들 크기 최소.
- ✅ 결정 단순함.
- ❌ envelope 패턴 사용자(Rails, JSON:API, 사용자 정의)는 모두 자체 strategy 작성 필요.
- ❌ "강력한 default 제공"이라는 메시지가 약화됨.
...
- ✅ 더 많은 페르소나 직접 커버.
- ❌ HAL/OData는 페이로드 직렬화 외 책임이 strategy를 넘어섬 (응답 hypermedia 처리, query 문법 등).
- ❌ 빌트인 프리셋이 9개를 넘으면 사용자 선택 부담 증가, 번들 크기 영향 비선형 증가.
- ❌ niche 컨벤션을 빌트인하면 메시지가 흐려짐 ("이 라이브러리는 무엇을 위한 것인가?").

## Consequences

### 긍정

- ✅ 6종 빌트인으로 시장 점유율 80% 이상 커버. SOM이 "RFC 6902 친화 백엔드"에서 "REST API를 사용하는 대다수 백엔드"로 확장.
- ✅ `postOnly` 빌트인이 한국 SI 트랙의 1급 시민화. PORTFOLIO §1의 "SI 통증" narrative와 코드가 일치.
- ✅ `envelope` 일반화 프리셋이 사용자 정의 envelope 패턴을 절반 이상 흡수 — 자체 strategy 작성 부담 감소.
- ✅ 본 ADR이 ADR-0002 (인터페이스 시그니처)의 입력 데이터로 작용. ctx 객체 필드 셋, serialize/decide 시그니처가 6종 프리셋을 모두 표현 가능해야 함.

### 감수해야 할 비용

- ⚠️ 빌트인 6종 각각의 단위 테스트 + e2e 시나리오 작성 부담. 매트릭스 = 6 프리셋 × (POST/PATCH/PUT) = 18 핵심 케이스 + 사용자 정의 케이스.
- ⚠️ HAL, OData, FHIR, GraphQL-REST 사용자에게 "사용자 정의 strategy 작성" 안내 문서 작성 필요. README 또는 별도 가이드.
- ⚠️ 점유율 추정치가 데이터 기반이 아닌 경험적 추정. 1년 이내 NPM 다운로드 데이터와 GitHub 이슈를 통해 검증 필요. 추정이 크게 틀린 경우 빌트인 셋 재조정.

### 후속 결정 (ADR로 분리 예정)

- ⏳ **ADR-0002 (Strategy 인터페이스 시그니처)** — 본 ADR의 6종 프리셋을 모두 표현 가능한 ctx 객체 필드 셋, serialize/decide 함수 시그니처 결정.
- ⏳ **ADR-0003 (Lazy Mode 호환성)** — 6종 프리셋이 lazy mode의 deepDiff 결과를 입력으로 받을 때 호환성 분석.
- ⏳ **ADR-0004 (Idempotency-Key 위치)** — 6종 중 `postOnly`는 PUT 멱등성 가정이 깨짐. Idempotency-Key 강제 권장 여부 결정.
