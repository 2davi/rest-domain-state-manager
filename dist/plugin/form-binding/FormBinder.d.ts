...
export type FromFormOptions = {
     ...
     * URL 설정 오버라이드. 미입력 시 `handler.getUrlConfig()` 폴백.
     */
    urlConfig?: import("../../model/DomainState.js").NormalizedUrlConfig | null | undefined;
    /**
     * 디버그 모드 활성화. `true`이면 Proxy 변경 시마다 디버그 채널에 broadcast.
     */
 ...
 * 타입 참조 전용 — 실제 구현은 `FormBinder.install()` 내부에 있다.
 */
export type FromFormFactory = (formOrId: string | HTMLFormElement, handler: import("../../src/handler/api-handler.js").ApiHandler, options?: FromFormOptions | undefined) => import("../../model/DomainState.js").DomainState;
/**
 * `FormBinder` 플러그인이 `DomainState.prototype`에 주입하는 인스턴스 메서드 시그니처.
 * 타입 참조 전용 — 실제 구현은 `FormBinder.install()` 내부에 있다.
 */
