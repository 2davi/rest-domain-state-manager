...
export class DomainPipeline {
     ...
     *     .run();
     */
    constructor(resourceMap: ResourceMap, { strict }?: PipelineOptions);
    /**
     * 병렬 fetch 대상 리소스 맵.
     ...
     */
    after(key: string, handler: AfterHandler): DomainPipeline;
    /**
 ...
 * 키는 리소스 식별자, 값은 `api.get()` 등이 반환하는 `Promise<DomainState>`.
 */
export type ResourceMap = Record<string, Promise<import("./DomainState.js").DomainState>>;
/**
 * `DomainPipeline` 생성자의 `options` 파라미터.
 */
 ...
 * `after()` 메서드에 전달하는 핸들러 함수 타입.
 */
export type AfterHandler = (domainState: import("./DomainState.js").DomainState) => void | Promise<void>;
/**
 * `_errors` 배열 및 `run()` 반환값의 `_errors` 프로퍼티에 포함되는 에러 항목.
 */
 ...
 * 성공한 리소스의 `DomainState` 맵에 선택적으로 `_errors` 배열이 포함된다.
 */
export type PipelineResult = Record<string, import("./DomainState.js").DomainState> & {
